...
export default function KamaiWidget() {
  ...
  const [messages, setMessages] = useState<Array<{ role: 'user' | 'assistant'; content: string }>>([
    { role: 'assistant', content: 'Я KAMAI — ИИ‑администратор. Чем помочь?' },
  ]);
  ...
  return (
    <>
      <button onClick={() => setOpen(!open)} className="fixed z-50 bottom-5 right-5 h-12 px-4 rounded-full bg-premium-gold text-premium-black font-bold shadow-lg">
        {open ? 'Закрыть KAMAI' : 'KAMAI • помощь'}
      </button>
      {open && (
        <div className="fixed z-50 bottom-20 right-5 w-[min(92vw,360px)] rounded-2xl bg-black/80 backdrop-blur border border-white/10 overflow-hidden">
          <div className="px-4 py-3 border-b border-white/10 font-bold">KAMAI — ИИ‑администратор</div>
          ...
          <div className="p-3 flex gap-2">
            <input value={input} onChange={e => setInput(e.target.value)} placeholder="Спросите у KAMAI…" className="flex-1 h-10 rounded-xl px-3 text-slate-900" />

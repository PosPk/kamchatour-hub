...
type Props = { showPoints?: boolean; className?: string };

export default function BearMap({ showPoints = false, className }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      map = L.map(mapRef.current, { zoomControl: false, attributionControl: false, dragging: false, scrollWheelZoom: false, doubleClickZoom: false, boxZoom: false, keyboard: false }).setView([56, 160], 5);
  ...
  return <div ref={mapRef} className={`w-full h-72 rounded-2xl overflow-hidden border border-white/10 bg-black ${className || ''}`} />;
}

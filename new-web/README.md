# Kamchatour Telegram Mini App (MVP)

## Быстрый старт

1) Установить зависимости Node 22.x

2) Сборка статики для веб:

```
npm run build
```

Результат: каталог `dist/` с редиректом на `/tg` и папкой `dist/tg` (страницы мини‑приложения).

3) Серверные функции (`/api/*`) деплоятся как serverless (Vercel).

## Переменные окружения

- TELEGRAM_BOT_TOKEN — токен бота (для валидации `initData`)
- CLOUDPAYMENTS_PUBLIC_ID — публичный ID CloudPayments
- CLOUDPAYMENTS_ENABLED — `1` чтобы включить кнопку оплаты
- CLOUDPAYMENTS_COMMISSION — десятичная комиссия (например `0.1`)
- OP_LEVEL_DEFAULT — `L1|L2|L3`

## Эндпоинты

- GET `/api/tg/tours` → `{ items }` из `public/partner-tours.json`
- GET `/api/tg/search?q=` → `{ items }`
- POST `/api/tg/auth` → `{ ok }` с проверкой HMAC и TTL
- POST `/api/tg/analytics` → `{ ok }`
- POST `/api/ai/recommend` → `{ items }` простой ранкер
- GET `/api/icons/noun?q=` → `{ icon }`
- GET `/api/tg/me` → `{ user }`
- POST `/api/tg/role` → `{ ok }`
- GET/POST `/api/tg/operator/level` → `{ level }`/`{ ok }`
- GET `/api/payment/config` → `{ enabled, publicId, commission, level }`
- POST `/api/webhooks/cloudpays` — вебхук CloudPayments (подпись X‑Signature)

## Безопасность

В `vercel.json` добавлены заголовки CSP/Referrer/Permissions для `/tg*`.

## Деплой

- Веб-статика: содержимое `dist/`
- API: папка `api/` как функции Vercel

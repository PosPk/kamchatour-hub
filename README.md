...
├── TotemContext.tsx  # Эко-баллы (локальные начисления)
...
- [x] Лента (локальная), аналитика событий, офлайн‑очередь
 - [x] Эко‑баллы (тотемы): локальные начисления + дневной лимит
 - [x] Трансфер: ядро БД (routes/stops/trips/seats/holds/tickets), seatmaps (bus/jeep), API (search/hold/book)
 - [x] AI‑чат: экран /ai, контекст, API‑прокси к OpenAI
...
- [ ] Эко‑баллы (Тотемы): серверная синхронизация через Supabase RPC
 - [ ] Трансфер: UI выбора мест и таймер удержания, билеты (QR)
